...
class UserMailer {
	...
	public static function send( $to, $from, $subject, $body, $replyto = null, $contentType = null ) {
		...
		if ( is_array( $wgSMTP ) ) {
			...
			$options = array();
			$options['from'] = $from->toString();

			if ( $wgEnotifImpersonal ) {
				$options['to'] = 'undisclosed-recipients:;';
			}
			else {
				$options['to'] = implode( ", ", (array )$dest );
			}

			//$options['cc'] 		= '';
			$options['smtp_host']		= isset($wgSMTP['smtp']) ? $wgSMTP['smtp'] : '';
			$options['smtp_port']		= isset($wgSMTP['port']) ? $wgSMTP['port'] : 25;
			$options['smtp_username']	= isset($wgSMTP['username']) ? $wgSMTP['username'] : '';
			$options['smtp_password']	= isset($wgSMTP['password']) ? $wgSMTP['password'] : '';
			$options['subject']		= $subject;//wfQuotedPrintable( $subject );
			$options['content']		= $body;
			$options['content_type']	= 'text';
			$options['charset']		= is_null( $contentType ) ? 'utf8' : $contentType;
			$options['tls']			= isset($wgSMTP['tls']) ? $wgSMTP['tls'] : false;
			//$options['compress']		= '';

			wfSuppressWarnings();

			// Create the mail object using the SaeMail method
			$mail_object = new SaeMail();

			wfDebug( "Sending mail via SaeMail to $dest\n" );
			$chunks = array_chunk( (array)$dest, $wgEnotifMaxRecips );
			foreach ( $chunks as $chunk ) {
				$options['to'] = implode( ", ", $chunk );
				$mail_object->setOpt( $options );
				$mailResult = $mail_object->send();
				# Based on the result return an error string,
				if ( $mailResult === false ) {
					wfDebug( "SaeMail failed: " . $mail_object->errmsg() . "\n" );
					$status = Status::newFatal( 'sea-mail-error', $mail_object->errmsg() );
				} else {
