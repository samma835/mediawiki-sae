...
class BitmapHandler extends ImageHandler {
	...
	function doTransform( $image, $dstPath, $dstUrl, $params, $flags = 0 ) {
		...
		$err = $this->transformSaeImage( $image, $scalerParams );// @Author Jieming Mao
/*
		switch ( $scaler ) {
			case 'im':
...
*/
	...
	/**
	 * Transform an image using the built in SaeImage
	 *
	 * @param $image File File associated with this thumbnail
	 * @param $params array Array with scaler params
	 *
	 ...
	 * @Author Jieming Mao
	 */
	protected function transformSaeImage( $image, $params ) {
		# Use SaeImage functions.
		#
		...
		$img = new SaeImage();
		if ( $img->setData( file_get_contents($params['srcPath']) ) === false ) {
			$err = $img->errno()." : ".$img->errmsg();
			wfDebug( "$err\n" );
			return $this->getMediaTransformError( $params, $err );
		}

		if ( $img->resize( $params['physicalWidth'], $params['physicalHeight'] ) === false ) {
			$err = $img->errno()." : ".$img->errmsg();
			wfDebug( "$err\n" );
			return $this->getMediaTransformError( $params, $err );
		}
		
		$new_data = $img->exec($typemap[$params['mimeType']]); // 执行处理并返回处理后的二进制数据
		//图片处理失败时输出错误码和错误信息
		if ( $new_data === false ) {
			$err = $img->errno()." : ".$img->errmsg();
			wfDebug( "$err\n" );
		...
		file_put_contents( $params['dstPath'], $new_data );
